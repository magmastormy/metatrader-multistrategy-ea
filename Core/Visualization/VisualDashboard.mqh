...
private:
    ...
    // Panel positions (column-based layout)
    int    m_col1X;   // Left column
    int    m_col2X;   // Middle column
    int    m_col3X;   // Right column
    int    m_startY;
    int    m_rowH;
...
public:
    ...
    // ensemble and onnx may be NULL if not registered
                ...
                double totalPnl)
    {
        ...
        // ========== COLUMN 1: System Health + Neural Net ==========
        row = 0;
        CreateRect("Bg1", m_col1X - 5, m_startY - 5, 300, 420, m_bgColor);
        ...
        CreateLabel("Health_EqVal", DoubleToString(eqRatio * 100.0, 1) + "%", m_col1X + 140, m_startY + row * m_rowH, GetHealthColor(eqRatio));
        DrawBar("Health_EqBar", m_col1X, m_startY + (row + 1) * m_rowH, eqRatio, 280, GetHealthColor(eqRatio));
        row += 2;

        // Balance / Equity
        CreateLabel("Sys_Bal", "Balance: $" + DoubleToString(balance, 0), m_col1X, m_startY + row * m_rowH, m_textColor);
        CreateLabel("Sys_Eq", "Equity: $" + DoubleToString(equity, 0), m_col1X + 150, m_startY + row * m_rowH, m_textColor);
        row++;
        ...
        CreateLabel("Sys_Trades", "Trades: " + (string)nnTotalTrades, m_col1X + 150, m_startY + row * m_rowH, m_textColor);
        row++;
        ...
        CreateLabel("Sys_WRVal", DoubleToString(winRate * 100.0, 1) + "%", m_col1X + 140, m_startY + row * m_rowH, GetConfidenceColor(winRate));
        DrawBar("Sys_WRBar", m_col1X, m_startY + (row + 1) * m_rowH, winRate, 280, GetConfidenceColor(winRate));
        row += 2;
        ...
        if(nn != NULL)
        {
            ...
            CreateLabel("NN_Steps", "Steps: " + (string)nn.GetTrainingSteps(), m_col1X + 150, m_startY + row * m_rowH, m_textColor);
            row++;
            ...
            CreateLabel("NN_SigVal", sigText + " @ " + DoubleToString(nnConf * 100.0, 1) + "%", m_col1X + 140, m_startY + row * m_rowH, sigColor);
            row++;

            // Conformal quantile
            double confQ = nn.GetLastUncertainty();
            CreateLabel("NN_ConfQ", "Conformal Q:", m_col1X, m_startY + row * m_rowH, m_textColor);
            CreateLabel("NN_ConfQVal", DoubleToString(confQ, 3), m_col1X + 140, m_startY + row * m_rowH, GetUncertaintyColor(confQ));
            DrawBar("NN_ConfBar", m_col1X, m_startY + (row + 1) * m_rowH, confQ, 280, GetUncertaintyColor(confQ));
            row += 2;
            ...
            CreateLabel("NN_BarV", "Vert Bars: " + (string)nn.GetBarrierVertBars(), m_col1X + 150, m_startY + row * m_rowH, m_textColor);
            row++;
        }
        else
        {
        ...
        // ========== COLUMN 2: Transformer / Brain ==========
        row = 0;
        CreateRect("Bg2", m_col2X - 5, m_startY - 5, 300, 420, m_bgColor);
        ...
        if(brain != NULL && brain.IsInitialized())
        {
            ...
            CreateLabel("Brain_UncVal", DoubleToString(unc * 100.0, 1) + "% [" + uncRec + "]", m_col2X + 140, m_startY + row * m_rowH, uncColor);
            DrawBar("Brain_UncBar", m_col2X, m_startY + (row + 1) * m_rowH, unc, 280, uncColor);
            row += 2;

            // Accuracy
            double acc = brain.GetAccuracy();
            CreateLabel("Brain_Acc", "Accuracy:", m_col2X, m_startY + row * m_rowH, m_textColor);
            CreateLabel("Brain_AccVal", DoubleToString(acc * 100.0, 1) + "%", m_col2X + 140, m_startY + row * m_rowH, GetConfidenceColor(acc));
            DrawBar("Brain_AccBar", m_col2X, m_startY + (row + 1) * m_rowH, acc, 280, GetConfidenceColor(acc));
            row += 2;
        ...
        row = 0;
        CreateRect("Bg3", m_col3X - 5, m_startY - 5, 300, 420, m_bgColor);

        CreateLabel("H3_Title", "AI CONFIGURATION", m_col3X, m_startY + row * m_rowH, m_headerColor, 11);
        row++;
        ...
        // ========== BOTTOM ROW: Summary ==========
        row = 19;
        CreateRect("BgBottom", m_col1X - 5, m_startY + row * m_rowH - 5, 905, 40, m_bgColor);
        CreateLabel("Bottom_Sep", "====================================================================================================", m_col1X, m_startY + row * m_rowH, m_dimColor);
        row++;

        string summary = "AI: " + (string)activeStrats + " active | Features: 65 | Health: " + ((eqRatio >= 0.97) ? "EXCELLENT" : (eqRatio >= 0.93) ? "CAUTION" : "CRITICAL");
        ...
        CreateLabel("Bottom_Time", timestamp, m_col3X + 100, m_startY + row * m_rowH, m_dimColor, 9);
