...
private:
    ...
    double CalculateMomentum() {
        ...
        if(m_rsiHandle != INVALID_HANDLE && BarsCalculated(m_rsiHandle) >= rsiPeriod) {
            double rsiValues[];
            ...
            PrintFormat("[WARN] CMarketAnalysis::CalculateMomentum - RSI data not ready or handle invalid for %s.", m_symbol);
        }
...
public:
    ...
    bool InitializeIndicators(const string symbolName, const ENUM_TIMEFRAMES period) {
        ...
        while(retryCount < maxRetries && !indicatorsReady) {
            bool adxReady = (m_adxHandle == INVALID_HANDLE) || BarsCalculated(m_adxHandle) > 0;
            bool atrReady = BarsCalculated(m_atrHandle) > 0;
            bool bbReady = BarsCalculated(m_bbHandle) > 0;
            bool macdReady = BarsCalculated(m_macdHandle) > 0;
            bool rsiReady = BarsCalculated(m_rsiHandle) > 0;
            bool ma20Ready = BarsCalculated(m_ma20Handle) > 0;
            bool ma50Ready = BarsCalculated(m_ma50Handle) > 0;
            bool ma200Ready = BarsCalculated(m_ma200Handle) > 0;
