...
public:
   ...
   //+------------------------------------------------------------------+
   bool CheckTierTransition(CUnifiedRiskManager &riskManager,
                            ...
                            CTradeManager &tradeMgr)
   {
      ...
      string milestoneMsg = CheckMilestone(m_previousEquity, currentEquity);
      ...
      int newTierIndex = FindTierIndex(currentEquity);
      ...
      if(newTierIndex != m_currentTierIndex)
      {
         ...
         ApplyTierToRiskManager(riskManager);
         ApplyTierToPositionSizer(positionSizer);

         // Alert
         Alert("COMPOUNDING TIER UPGRADE: Account $", DoubleToString(currentEquity, 2),
               " | New tier: ", GetTierName(m_currentTierIndex),
               " | Risk: ", DoubleToString(m_tiers[m_currentTierIndex].riskPerTradeOverride, 1), "%");

         m_previousEquity = currentEquity;
         return true;
      }
   ...
   //+------------------------------------------------------------------+
   void ApplyTierToPositionSizer(CPositionSizer &positionSizer)
   {
      if(m_currentTierIndex < 0 || m_currentTierIndex >= m_tierCount)
         return;
      ...
      SPositionSizingParams params = positionSizer.GetParameters();
      params.riskPercent = tier.riskPerTradeOverride;
      positionSizer.SetParameters(params);
   ...
   //+------------------------------------------------------------------+
   string CheckMilestone(double previousEquity, double currentEquity)
   {
      double milestones[] = {25.0, 50.0, 100.0, 200.0, 500.0};
      
      for(int i = 0; i < ArraySize(milestones); i++)
      {
         if(previousEquity < milestones[i] && currentEquity >= milestones[i])
         {
